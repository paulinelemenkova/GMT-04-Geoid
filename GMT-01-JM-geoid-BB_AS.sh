...
gdalinfo EGM2008ner1.grd -stats
...
gmt makecpt -Cmag.cpt -V -T-110/80/2 > colors.cpt

# Generate a file
ps=Geoid_BB.ps
# Make raster image
gmt grdimage EGM2008ner1.grd -Ccolors.cpt -R74/100/2/23 -JM6i -P -I+a15+ne0.75 -Xc -K > $ps
gmt grdimage EGM2008ner2.grd -Ccolors.cpt -R74/100/2/23 -JM6i -P -I+a15+ne0.75 -Xc -O -K >> $ps

# Add grid
gmt psbasemap -R -J \
    -Bpx10f2.5a5 -Bpyg5f2.5a5 -Bsxg5 -Bsyg5 \
    --MAP_TITLE_OFFSET=0.8c \
    --FONT_TITLE=12p,Helvetica,black \
    --FONT_ANNOT_PRIMARY=7p,Helvetica,black \
    -B+t"Geoid geopotential model: Bay of Bengal and Andaman Sea" -O -K >> $ps
...
gmt grdcontour EGM2008ner1.grd -R -J -C2 -A4 -Wthin,dimgray -O -K >> $ps
gmt grdcontour EGM2008ner2.grd -R -J -C2 -A4 -Wthin,dimgray -O -K >> $ps
...
gmt pscoast -R -J -P \
    -Ia/thinnest,blue -N1/thinner,red -Wthin -Df -O -K >> $ps
...
gmt psscale -Dg74/-0.3+w15.2c/0.4c+h+o0.0/0i+ml -R -J -Ccolors.cpt \
    ...
    -Bg5f2a10+l"Color scale 'mag' [C=RGB, -110/80/5]" \
    -I0.2 -By+lm -O -K >> $ps
#
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f14p,Helvetica,black+jLB >> $ps << EOF
75.5 17.4 I  N  D  I  A
EOF
gmt pstext -R -J -N -O -K \
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f9p,Helvetica,blue+jLB+a-350 >> $ps << EOF
...
2.0 17.0 Global geoid image EGM2008-WGS84 2,5 minute resolution
EOF

# Convert to image file using GhostScript
gmt psconvert Geoid_BB.ps -A0.8c -E720 -Tj -Z
