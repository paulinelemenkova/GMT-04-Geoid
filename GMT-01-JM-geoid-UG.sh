...
# Generate a file
ps=Geoid_UG.ps
gmt grdimage geoid_TZ.grd -Cwysiwyg -R29/35.5/-1.5/4.3 -JM6.5i -P -Xc -I+a15+ne0.75 -K > $ps
gmt grdimage geoid_ET.grd -Cwysiwyg -R29/35.5/-1.5/4.3 -JM6.5i -P -Xc -I+a15+ne0.75 -O -K >> $ps

# Add shorelines
gmt grdcontour geoid_TZ.grd -R -J -C0.25 -A1+f9p,25,black -Wthinner,dimgray -O -K >> $ps
gmt grdcontour geoid_ET.grd -R -J -C0.25 -A1+f9p,25,black -Wthinner,dimgray -O -K >> $ps
...
gmt psscale -Dg29.0/-2.0+w16.5c/0.15i+h+o0.3/0i+ml+e -R -J -Ccolors.cpt \
    ...
    -Bg4f0.2a2+l"Color scale wysiwyg: 20 well-separated RGB colors [C=RGB, -T0/40/1]" \
    -I0.2 -By+lm -O -K >> $ps
