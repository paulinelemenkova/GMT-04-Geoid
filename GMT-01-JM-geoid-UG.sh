...
gmt grdconvert n00e00/w001001.adf geoid_TZ.grd
...
# gmt makecpt --help
#gmt makecpt -Cwysiwyg -T-20/-5 > colors.cpt
gmt makecpt -Cnjmag -T-18/-5 -Ic > colors.cpt
#gmt makecpt -Cncar -T-20/-5 > colors.cpt
#-Ic Reverse sense of color table spectrum
# haxby

# Generate a file
ps=Geoid_UG.ps
gmt grdimage geoid_TZ.grd -Ccolors.cpt -R29/35.5/-1.5/4.3 -JM6.5i -P -Xc -I+a15+ne0.75 -K > $ps
gmt grdimage geoid_ET.grd -Ccolors.cpt -R29/35.5/-1.5/4.3 -JM6.5i -P -Xc -I+a15+ne0.75 -O -K >> $ps

# Add shorelines
gmt grdcontour geoid_TZ.grd -R -J -C0.25 -A0.5+f9p,25,black -Wthinner,dimgray -O -K >> $ps
...
gmt psbasemap -R -J \
    ...
    -B+t"Geoid gravitational model of Uganda" -O -K >> $ps
...
gmt psscale -Dg29.0/-2.0+w16.5c/0.15i+h+o0.3/0i+ml+e -R -J -Ccolors.cpt \
    ...
    -Bg2f0.2a1+l"Color scale 'njmag' by New Jersey Geological Survey [C=RGB -T-18/-5]" \
    -I0.2 -By+lm -O -K >> $ps
...
EOF
gmt pstext -R -J -N -O -K \
-F+f9p,0,black+jLB+a-345 >> $ps << EOF
32.62 0.45 Kira
EOF
gmt psxy -R -J -Sc -W0.5p -Gyellow -O -K << EOF >> $ps
32.63 0.40 0.20c
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f9p,26,blue2+jLB+a-50 >> $ps << EOF
32.57 2.8 Acuwa
EOF
gmt pstext -R -J -N -O -K \
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f8p,26,blue2+jLB+a-295 >> $ps << EOF
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f9p,26,blue2+jLB+a-350 >> $ps << EOF
30.65 0.25 Katonga
EOF
...
gmt psconvert Geoid_UG.ps -A0.5c -E720 -Tj -Z
