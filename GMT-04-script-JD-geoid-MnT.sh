...
# Equidistant conic projection (here: Manila Trench).
...
gmt grdimage geoid.egm96.grd -I+a45+nt1 \
    -R105/123/8/24 -JD114/13/16/19/6i -Cgeoid.cpt -P -K > $ps
# Step-5. Add basemap: grid, title, costline
gmt pscoast -R -J -P \
	-V -W0.25p \
    -Df -B+t"Geoid gravitational regional model: Manila Trench area" \
	-Bxg3f4a2 -Byg3f2a2 \
    -O -K >> $ps
...
gmt psbasemap -R -J \
    ...
    -Lx13c/-2.6c+c50+w500k+l"Equidistant conic projection. Scale, km"+f \
    -UBL/-15p/-70p -O -K >> $ps
...
gmt psbasemap -R -J \
    ...
    --MAP_LABEL_OFFSET=0.1c \
    -Tmg112/12+w3.4c+d-14.5+t45/10/5+ithin,blue+p0.1p,red+l+jCM \
    -O -K >> $ps
# Step-9. Add color legend
gmt psscale -R -J -Cgeoid.cpt\
    -DjBC+o0.8c/-2.0c+w12c/0.5c+h\
    --FONT_LABEL=7p,Palatino-Roman,dimgray \
...
# Step-11. Add logo
gmt logo -R -J -Dx6.5/-3.4+o0.1i/0.1i+w2c -O -K >> $ps
# Step-13. Add text
gmt pstext -R -J -X2.5c -Y-7.4c -N -O -K \
    -F+f7p,Palatino-Roman,dimgray+jCB >> $ps << END
10.0 0.0 Standard paralles at 13\232 and 19\232 N
END
# Step-14. Add text
gmt pstext -R -J -N -X8.0c -Y1.0c -O \
    -F+f8p,Palatino-Roman,dimgray+jCB >> $ps << END
1.0 1.0 Magnetic rose
END
# Step-15. Convert to image file using GhostScript
gmt psconvert Geoid_MnT.ps -A0.7c -E720 -Tj -Z
