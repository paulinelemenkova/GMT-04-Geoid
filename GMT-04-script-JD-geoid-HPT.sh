...
gmt psscale -R -J -Cgeoid.cpt\
    ...
    -I0.2 -By+lmGal -UBL/2p/-90p -O -K >> $ps
# texts
...
EOF
# Step-11. Add logo
gmt logo -Dx6.5/-4.0+o0.1i/0.1i+w2c -O -K >> $ps
# Step-12. Add subtitle
gmt pstext -R0/10/0/15 -JX10/10 -X0.5c -Y11.8c -N -O \
    -F+f10p,Helvetica,black+jLB >> $ps << EOF
2.0 11.5 Data source of geoid reference: EGM96 Earth Gravitational Model
2.0 11.0 Lambert Azimuthal Equal-Area projection. Center lon/lat: 165\232W/45\232S
EOF
# Step-15. Convert to image file using GhostScript
gmt psconvert Geoid_HPT.ps -A1.5c -E720 -Tj -Z
