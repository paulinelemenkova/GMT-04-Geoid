...
# Generate a file
ps=Geoid_TZ.ps
gmt grdimage geoid_TZ.grd -Cwysiwyg -R29/42/-13/0 -JM6.5i -P -Xc -I+a15+ne0.75 -K > $ps
...
-F+f9p,13,black+jLB -Gwhite@50 >> $ps << EOF
...
# Lakes
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,13,blue+jLB+a-80 -Gwhite@50 >> $ps << EOF
29.5 -5.1 Lake
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,13,blue+jLB+a-65 -Gwhite@50 >> $ps << EOF
30.1 -6.8 Tanganyika
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f10p,13,blue+jLB -Gwhite@50 >> $ps << EOF
32.2 -1.2 Lake
32.2 -1.5 Victoria
EOF
gmt pstext -R -J -N -O -K \
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f8p,25,white+jLB -Gdimgray@60 >> $ps << EOF
29.2 -7.5 CONGO
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f8p,25,white+jLB -Gdimgray@60 >> $ps << EOF
...
EOF
gmt pstext -R -J -N -O -K \
-F+jTL+f12p,13,white+jLB >> $ps << EOF
40.5 -8.5 Indian
40.5 -9.5 Ocean
EOF
