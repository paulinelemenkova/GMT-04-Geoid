...
grdconvert n45w135/ EGM2008BS1.grd
grdconvert n45w180/ EGM2008BS2.grd

# Extract subset of img file in Mercator or Geographic format
#img2grd grav_27.1.img -R180/270/60/80 -Ggrav.grd -T1 -I1 -E -S0.1 -V

gdalinfo EGM2008BS1.grd -stats
gdalinfo EGM2008BS2.grd -stats
# {-49.45299911499023,17.33600044250488}
#{-48.72900009155273,49.64400100708008}

# Select a color palette
gmt makecpt -Chaxby.cpt -V -T-50/21/1 > colors.cpt

# Generate a file
ps=Geoid_BS.ps

# Make raster image
gmt grdimage EGM2008BS1.grd -I+a45+nt1 -R180/270/60/80 -JM5.5i -Ccolors.cpt -P -K > $ps
gmt grdimage EGM2008BS2.grd -I+a45+nt1 -R180/270/60/80 -JM5.5i -Ccolors.cpt -P -O -K >> $ps
#gmt grdimage geoid.egm96.grd -I+a45+nt1 -R180/270/60/80 -JM5.5i -Ccolors.cpt -P -K > $ps
...
gmt psscale -Dg169/60.0+w10.0c/0.4c+v+o0.3/0i+ml -R -J -Ccolors.cpt \
    ...
    --FONT_ANNOT_PRIMARY=6p,Helvetica,black \
    -B10f2+l"Color scale: Haxby: Bill Haxby's color scheme for for geoid heights, m [C=RGB])" \
    -I0.2 -By+lm -O -K >> $ps
...
1.7 7.5 World geoid image 2,5 minute resolution, EGM2008-WGS 84
EOF

# Convert to image file using GhostScript
gmt psconvert Geoid_BS.ps -A1.5c -E720 -Tj -Z
